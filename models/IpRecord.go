...
func (ipRecord *IpRecord) Create(db *sql.DB) (sql.Result, error) {
	...
	defer tx.Rollback()
	stmt, errPrepare  := tx.Prepare("INSERT INTO hits(ip, hit_count) VALUES(?,?)")
	if errPrepare != nil {
...
func (ipRecord *IpRecord) Read(db *sql.DB) (*IpRecord, error){
	...
	row := db.QueryRow("SELECT * FROM hits WHERE ip = ?", ipRecord.ip)
...
func (ipRecord *IpRecord) Update(db *sql.DB) (sql.Result, error){
	...
	defer tx.Rollback()
	stmt, errPrepare  := tx.Prepare("UPDATE hits SET hit_count=? WHERE ip =?")
	if errPrepare != nil {
		return nil, errPrepare
	}
	defer stmt.Close()
...
func (ipRecord *IpRecord) Delete(db *sql.DB) (*IpRecord, error){
	...
	defer tx.Rollback()
	stmt, errPrepare  := tx.Prepare("DELETE FROM hits WHERE ip =?")
	if errPrepare != nil {
		return nil, errPrepare
	}
	defer stmt.Close()
